...
| `JSON.parse` / `JSON.stringify` | ✅ | `parse` is a strict recursive-descent parser (not `eval`-based) — rejects non-JSON input and does not reconstruct functions; `stringify` produces compact output (no whitespace), matching `JSON.stringify(x)` |
| `parseInt` / `parseFloat` | ✅ | |
| `isNaN` / `isFinite` | ✅ | |
